...
#include <unistd.h> // necessária para sleep
#include "funcaux.h"
#include "filechange.h" // necessária para listar_cartas
 ...
 */
int modo(){
    printf("\n\033[5mEscolha o modo de jogo:\n\033[1;93m1 - NORMAL\n\033[1;31m2 - DIFÍCIL\033[m\n: ");

    return get_int(1,2, "\033[1;91mModo inválido!\033[1m Insira 1 ou 2:\033[m ");
}
 ...
 * @param cartas_usadas // vetor de posições das cartas usadas (posições em 'cartas')
 * @return Cartas* 
 */
Cartas* get_baralho(Cartas* cartas, int quantd_cartas, int quant_cartas_baralho, int* quant_cartas_usadas, int** cartas_usadas) {
    ...
    if(baralho == NULL){
        perror("\n\033[1;91mErro ao alocar memoria para o baralho\033[m");
        exit(1);
    }

    int usadas = 0;
    if(*quant_cartas_usadas > 0) usadas = 1;

    if(!usadas){
        *cartas_usadas = (int*) malloc(quant_cartas_baralho * sizeof(int));
        if(*cartas_usadas == NULL){
            perror("\n\033[1;91mErro ao alocar memoria para cartas_usadas\033[m");
            exit(1);
        }
        ...
        *quant_cartas_usadas = quant_cartas_baralho;
        ...
        for (int i = 0; i < quantd_cartas; i++) {
            if(!verify_int_in_vetor(*cartas_usadas, *quant_cartas_usadas, i)){
                baralho[j] = cartas[i];
                j++;
                if(j >= quant_cartas_baralho) break;
            }
        }
        *quant_cartas_usadas += quant_cartas_baralho;
        *cartas_usadas = realloc(*cartas_usadas, *quant_cartas_usadas * sizeof(int));
        if(*cartas_usadas == NULL){
            perror("\n\033[1;91mErro ao realocar memoria para cartas_usadas\033[m");
            exit(1);
        }
        for (int i = 0; i < quant_cartas_baralho; i++) {
    ...
    //free será chamado na main em main.c

    return baralho;
}

/**
 * @brief Retorna o primeiro player a jogar. Caso 0, cpu começa, caso 1, player começa.
 * 
 * @return int 
 */
int primeiro_player(){
...
int verifica_maior(Cartas* carta_jogador, Cartas* carta_cpu, int atributo){
    ...
    {
    case FORCA:
        if (carta_jogador->forca > carta_cpu->forca) return 1;
        if (carta_jogador->forca < carta_cpu->forca) return 0;
        return -1;
    case HABILIDADE:
        if (carta_jogador->habilidade > carta_cpu->habilidade) return 1;
        if (carta_jogador->habilidade < carta_cpu->habilidade) return 0;
        return -1;
    case VELOCIDADE:
        if (carta_jogador->velocidade > carta_cpu->velocidade) return 1;
        if (carta_jogador->velocidade < carta_cpu->velocidade) return 0;
        return -1;
    case PODERES:
        if (carta_jogador->poderes > carta_cpu->poderes) return 1;
        if (carta_jogador->poderes < carta_cpu->poderes) return 0;
        return -1;
    case CURA:
        if (carta_jogador->poder_cura > carta_cpu->poder_cura) return 1;
        if (carta_jogador->poder_cura < carta_cpu->poder_cura) return 0;
        return -1;
    default:
        return -1;
    }
...
void lidar_com_empate(Cartas** baralho_jogador, int* quant_cartas_jogador, Cartas** baralho_cpu, int* quant_cartas_cpu, Cartas** cartas_empate_jogador, Cartas** cartas_empate_cpu, int* quant_cartas_empate) {
    ...
    *cartas_empate_cpu = (Cartas*) realloc(*cartas_empate_cpu, (*quant_cartas_empate) * sizeof(Cartas));
    ...
    if (*quant_cartas_jogador > 0) {
        ...
        if (!temp_jogador) {
            perror("\n\033[1;91mErro ao realocar memoria para baralho_jogador\033[m");
            exit(1);
        }
    ...
    if (*quant_cartas_cpu > 0) {
        ...
        if (!temp_cpu) {
            perror("\n\033[1;91mErro ao realocar memoria para baralho_cpu\033[m");
            exit(1);
        }
 ...
 *
 * Esta função gerencia o fluxo de uma partida completa, incluindo a seleção de atributos,
 * comparação de cartas, tratamento de empates e transferência de cartas entre os baralhos.
 * A partida continua até que um dos jogadores fique sem cartas.
 *
...
void partida(Cartas* baralho_jogador, Cartas* baralho_cpu, int quant_cartas_baralho){
    if(sistema()) system("cls");
    else system("clear");

    int player_joga = 0; // booleano -> 0 = cpu joga, 1 = player joga
    int win = 0; // booleano -> 0 = ninguém ganhou, 1 = alguém ganhou -> fim de jogo
    ...
    if(primeiro_player()){
        printf("\n\033[1;32mVocê começa!\033[m");
        player_joga = 1;
    }

    while(!win){ // começando a partida
        ...
        if(player_joga){
            ...
            printf("\nEscolha um atributo:\n1 - Força\n2 - Habilidade\n3 - Velocidade\n4 - Poderes\n5 - Cura\n: ");
            atributo = get_int(1, 5, "\033[1;91mAtributo inválido!\033[m Insira um número de 1 a 5:\033[m ");
            printf("\nAtributo maior ou menor?\n1 - Maior\n0 - Menor\n: ");
            maior_menor = get_int(0, 1, "\033[1;91mEntrada inválida!\033[m Insira 0 ou 1:\033[m ");
        }else{
            srand(time(NULL));
        ...
        switch (atributo) {
            case FORCA: atributo_nome = "For�a"; break;
            case HABILIDADE: atributo_nome = "Habilidade"; break;
            case VELOCIDADE: atributo_nome = "Velocidade"; break;
            case PODERES: atributo_nome = "Poderes"; break;
            case CURA: atributo_nome = "Cura"; break;
            default: atributo_nome = "Desconhecido"; break;
        }
        ...
        if(quem_ganhou == -1){
            ...
            printf("\n\033[1;32mVocê venceu a rodada!\033[m");
        ...
        if(quant_cartas_cpu == 0 || quant_cartas_jogador == 0) win = 1;
        player_joga = !player_joga;
        sleep(1);
    }
    char vencedor[10];
    int pontuacao = 10*((100*vitorias)/rodadas);
    Historico partidaHist;
    
    if(quant_cartas_cpu == 0){
        printf("\n\n\033[7;32mVocê venceu o jogo!\033[m\n\n");
        strcpy(vencedor, "Jogador");
    }else{
    ...
    printf("\n\033[1;32mEstatasticas da partida:\nrodadas: %i\nvitorias: %i\nempates: %i\npontua��o: %i pontos\033[m",
    partidaHist.rodadas, partidaHist.vitorias, partidaHist.empates, partidaHist.pontuacao);


    //verificar se houve outras partidas tentando abrir um historico.csv
    ...
    if(historico == NULL){
        ...
        //escrevendo o primeiro registro
